...
def test_lockf_exclusive():
    TestFcntl.f = open(TESTFN, 'wb+')
    cmd = test_fcntl.fcntl.LOCK_EX | test_fcntl.fcntl.LOCK_NB
    test_fcntl.fcntl.lockf(TestFcntl.f, cmd)
    p = Process(target=test_fcntl.try_lockf_on_other_process_fail, args=(TESTFN, cmd))
    p.start()
    p.join()
