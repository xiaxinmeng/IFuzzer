...
def test_logger_disabling():
    ConfigFileTest.apply_config(ConfigFileTest.disable_test)
    logger = logging.getLogger('some_pristine_logger')
    ConfigFileTest.assertFalse(logger.disabled)
    ConfigFileTest.apply_config(ConfigFileTest.disable_test)
    ConfigFileTest.assertTrue(logger.disabled)
    ConfigFileTest.apply_config(ConfigFileTest.disable_test, disable_existing_loggers=False)
    ConfigFileTest.assertFalse(logger.disabled)
