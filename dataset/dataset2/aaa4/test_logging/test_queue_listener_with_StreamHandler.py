...
def test_queue_listener_with_StreamHandler():
    listener = logging.handlers.QueueListener(QueueHandlerTest.queue, QueueHandlerTest.root_hdlr)
    listener.start()
    try:
        1 / 0
    except ZeroDivisionError as e:
        exc = e
        QueueHandlerTest.que_logger.exception(QueueHandlerTest.next_message(), exc_info=exc)
    listener.stop()
    QueueHandlerTest.assertEqual(QueueHandlerTest.stream.getvalue().strip().count('Traceback'), 1)
