...
def test_should_not_rollover():
    rh = logging.handlers.RotatingFileHandler(RotatingFileHandlerTest.fn, maxBytes=0)
    RotatingFileHandlerTest.assertFalse(rh.shouldRollover(None))
    rh.close()

RotatingFileHandlerTest = test_logging.RotatingFileHandlerTest()
RotatingFileHandlerTest.setUp()
test_should_not_rollover()
