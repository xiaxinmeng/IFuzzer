...
def test_reconfigure_encoding_read():
    data = 'abcé\n'.encode('latin1') + 'déf\n'.encode('utf8')
    raw = TextIOWrapperTest.BytesIO(data)
    txt = TextIOWrapperTest.TextIOWrapper(raw, encoding='latin1', newline='\n')
    TextIOWrapperTest.assertEqual(txt.readline(), 'abcé\n')
    with TextIOWrapperTest.assertRaises(TextIOWrapperTest.UnsupportedOperation):
        txt.reconfigure(encoding='utf-8')
    with TextIOWrapperTest.assertRaises(TextIOWrapperTest.UnsupportedOperation):
        txt.reconfigure(newline=None)

TextIOWrapperTest = test_io.TextIOWrapperTest()
test_reconfigure_encoding_read()
