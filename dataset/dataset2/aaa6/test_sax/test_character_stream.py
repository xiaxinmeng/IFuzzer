...
def test_character_stream():
    src = InputSource(PrepareInputSourceTest.file)
    src.setCharacterStream(PrepareInputSourceTest.make_character_stream())
    prep = prepare_input_source(src)
    PrepareInputSourceTest.assertIsNone(prep.getByteStream())
    PrepareInputSourceTest.checkContent(prep.getCharacterStream(), 'This is a character stream.')
