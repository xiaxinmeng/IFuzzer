...
def test_expat_incomplete():
    ...
    ErrorReportingTest.assertRaises(SAXParseException, parser.parse, StringIO('<foo>'))
    ErrorReportingTest.assertEqual(parser.getColumnNumber(), 5)
    ErrorReportingTest.assertEqual(parser.getLineNumber(), 1)
