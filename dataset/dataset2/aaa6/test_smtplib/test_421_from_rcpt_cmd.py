...
def test_421_from_rcpt_cmd():
    smtp = smtplib.SMTP(test_smtplib.HOST, SMTPSimTests.port, local_hostname='localhost', timeout=support.LOOPBACK_TIMEOUT)
    smtp.noop()
    SMTPSimTests.serv._SMTPchannel.rcpt_response = ['250 accepted', '421 closing']
    with SMTPSimTests.assertRaises(smtplib.SMTPRecipientsRefused) as r:
        smtp.sendmail('John', ['Sally', 'Frank', 'George'], 'test message')
    SMTPSimTests.assertIsNone(smtp.sock)
    SMTPSimTests.assertEqual(SMTPSimTests.serv._SMTPchannel.rset_count, 0)
    SMTPSimTests.assertDictEqual(r.exception.args[0], {'Frank': (421, b'closing')})
