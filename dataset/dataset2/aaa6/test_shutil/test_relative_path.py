...
def test_relative_path():
    (base_dir, tail_dir) = os.path.split(TestWhich.dir)
    with os_helper.change_cwd(path=base_dir):
        rv = shutil.which(TestWhich.file, path=tail_dir)
        TestWhich.assertEqual(rv, os.path.join(tail_dir, TestWhich.file))
