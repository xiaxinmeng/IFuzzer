...
def test_filesystem_full():
    with unittest.mock.patch(_ZeroCopyFileTest.PATCHPOINT, side_effect=OSError(errno.ENOSPC, 'yo')):
        with _ZeroCopyFileTest.get_files() as (src, dst):
            _ZeroCopyFileTest.assertRaises(OSError, _ZeroCopyFileTest.zerocopy_fun, src, dst)
