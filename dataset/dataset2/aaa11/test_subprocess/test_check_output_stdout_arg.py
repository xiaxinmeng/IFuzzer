...
def test_check_output_stdout_arg():
    with ProcessTestCase.assertRaises(ValueError) as c:
        output = subprocess.check_output([sys.executable, '-c', "print('will not be run')"], stdout=sys.stdout)
        ProcessTestCase.fail('Expected ValueError when stdout arg supplied.')
    ProcessTestCase.assertIn('stdout', c.exception.args[0])
