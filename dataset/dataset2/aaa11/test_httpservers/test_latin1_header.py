...
def test_latin1_header():
    BaseHTTPServerTestCase.con.request('LATINONEHEADER', '/', headers={'X-Special-Incoming': 'Ärger mit Unicode'})
    res = BaseHTTPServerTestCase.con.getresponse()
    BaseHTTPServerTestCase.assertEqual(res.getheader('X-Special'), 'Dängerous Mind')
    BaseHTTPServerTestCase.assertEqual(res.read(), 'Ärger mit Unicode'.encode('utf-8'))
