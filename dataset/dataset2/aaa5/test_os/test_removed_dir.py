...
def test_removed_dir():
    path = os.path.join(TestScandir.path, 'dir')
    os.mkdir(path)
    entry = TestScandir.get_entry('dir')
    os.rmdir(path)
    if os.name == 'nt':
        TestScandir.assertTrue(entry.is_dir())
    TestScandir.assertFalse(entry.is_file())
